      ...
      <div className={styles['container']}>
        ...
        <div className={styles['header-actions']}>
          ...
          {!isLogged && (
            <Link to="/auth/register" className={styles['navbar-link']}>
              <button className={`${styles['btn']} ${styles['btn-primary']}`}>{t('Sign in')}</button>
            </Link>
          )}
        </div>
        ...
        <nav className={`${styles['navbar']} ${isNavbarActive ? styles['active'] : ''}`} data-navbar>
          ...
          <ul className={styles['navbar-list']}>
            <li>
              <Link to="/" className={styles['navbar-link']}>{t('Home')}</Link>
            </li>

            <li className={styles['dropdown']}>
              <button className={styles['dropbtn']}>
                <Link to="/events/" className={styles['navbar-link']}>{t('Events')} <i className='bx bxs-chevron-down'></i></Link>
              </button>
              <div className={styles['dropdown-content']}>
                <Link to="/events/theater" className={styles['navbar-link']}>{t('Theater')}</Link>
                <Link to="/events/concerts" className={styles['navbar-link']}>{t('Concerts')}</Link>
              </div>
            </li>
            ...
            {isLogged && (
              <>
                <li>
                  <Link to="/search" className={styles['navbar-link']}>{t('Search')}</Link>
                </li>

                {user?.role === 'user' && (
                  <li>
                    <Link to="/profile" className={styles['navbar-link']}>{t('Profile')}</Link>
                  </li>
                )}
                ...
                {user?.role === 'admin' && (
                  <>
                    <li>
                      <Link to="/users" className={styles['navbar-link']}>{t('Users')}</Link>
                    </li>
                    <li>
                      <Link to="/events/add" className={styles['navbar-link']}>{t('Add Event')}</Link>
                    </li>
                  </>
                )}

                <li>
                  <a onClick={ logout } style={{ cursor: 'pointer' }} className={styles['navbar-link']}>{t('Logout')}</a>
                </li>
              </>
            )}
            ...
            {!isLogged && (
              <>
                <li>
                  <Link to="/auth/register" className={styles['navbar-link']}>{t('Register')}</Link>
                </li>
                <li>
                  <Link to="/auth/login" className={styles['navbar-link']}>{t('Login')}</Link>
                </li>
              </>
            )}

            <li>
              <Link to="/contacts" className={styles['navbar-link']}>{t('Contacts')}</Link>
            </li>
            <li>
              <Link to="/about" className={styles['navbar-link']}>{t('About')}</Link>
            </li>
          </ul>
